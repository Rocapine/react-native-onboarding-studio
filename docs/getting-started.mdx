...
The SDK automatically includes `react-native-reanimated`, `react-native-gesture-handler`, and `react-native-svg` as dependencies, so you don't need to install them separately.
:::
...
:::tip Progress Bar Included
The progress bar is **automatically included** in `OnboardingProvider` - no need to add it manually! It will show/hide based on your step configuration.
:::

### Step 2: Create an Onboarding Screen
...
| `cacheKey` | `string` | `"rocapine-onboarding-studio"` | AsyncStorage key |
